...
class Siamese_Loader:
    #this function i for loding the batches and testing tsks to a siames net
    def __init__(self, path, data_subsets=["train", "val"]):
        ...
        print("Printing filestatement",self.file_set)
    ...
    def get_batch(self, batch_size, s="train"):
        ...
        #This function creats batch of n pairs, half are in one classs and the other half in a different class
        if s=='train':
          ids = self.train_set
        else:
          ids = self.val_set
    ...
    def generate(self, batch_size, s="train"):
        #this function generates  batchs, so that the model.fit generator can be used.
        while True:
            pairs, targets = self.get_batch(batch_size,s)
            yield (pairs, targets)    

    def make_oneshot_task(self, N, s="val"):
        #this function creats a pasre of test image, suppport wet fro testing Nway one-shot learning.
        if s=='train':
          ids = self.train_set
        else:
          ids = self.val_set
    ...
    def test_oneshot(self, model, N, k, s="val", verbose=0):
        # THis function test the average N way oneshot learning accuracy of a siamese neural net over k one-shot tasks
        n_correct = 0
        if verbose:
